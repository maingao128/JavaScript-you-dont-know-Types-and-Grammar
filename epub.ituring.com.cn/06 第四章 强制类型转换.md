...
`第一章`里我们说过，强制类型转换是一个设计上的缺陷还是一个有用的特性，这个争论从JS诞生之日起就有了。许多JS书籍将强制类型转换描述为危险、难懂、糟糕的设计。

这里我们秉承本系列丛书的一贯宗旨，对于不懂的地方应该有困难要上，没有困难创造困难也要上，不会因为种种传言和挫折就退避三舍。

本章旨在全面介绍强制类型转换的优点（的确有！）和缺点，让你能够在开发中合理运用它。
...
<table>
    <tr style="border: none;">
        ...
        <td style="border: none;">JavaScript中的强制类型转换总是返回标量基本类型值（见<b>第二章</b>），如：<b>string</b>，<b>number</b>，和<b>boolean</b>，不会返回<b>object</b>和<b>function</b>。在<b>第三章</b>中我们介绍过“封装”，就是为标量基本类型值封装一个相应类型的对象，但这并非严格意义上的强制类型转换。</td>
    </tr>
</table>
