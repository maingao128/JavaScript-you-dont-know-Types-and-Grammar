...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img width="1000px" src="/api/storage/getbykey/screenshow?key=1604a41b2abfdacaddba"></td>
...
<table>
    <tr style="border: none;">
        ...
        <td style="border: none;">从技术角度来解释要更复杂一些。ES5规范12.2节“变量语句”中的变量声明（`VariableDeclaration`）算法实际上有一个返回值（是一个包含所声明变量名称的字符串，很奇特吧？），但是这个值被变量语句（`VariableStatement`）算法屏蔽掉了（`for..in`循环除外），最后返回结果为空（即`undefined`）。</td>
    </tr>
</table>

如果你用开发控制台（或者JavaScript REPL——read/evaluate/print/loop工具）调试过代码，应该会看到很多语句的返回值显示为`undefined`，只是你可能从未探究过其中的原因。其实，控制台中显示的就是语句的结果值。

但在代码中是没有办法获得这个结果值的，其具体解决方法比较复杂，我们首先得弄清楚为什么要获得语句的结果值。
...
但也不是没有办法，可以使用`,`语句系列逗号运算符（statement-series comma operator）。它可以将多个独立的表达式语句串联成一个语句：
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">根据运算符优先级，`a++,  a`需要放到`( .. )`中。本章后面将会介绍。</td>
    </tr>
</table>

`a++, a`中的第二个表达式`a`在`a++`之后执行，结果为`43`，并被赋值给`b`。

再如`delete`运算符。第2章中曾提及，`delete`被用来删除`object`中的属性或`array`中的单元。通常它以独立语句的形式出现：
...
如果操作成功，`delete`返回`true`，否则返回`false`。其副作用是属性被从`object`中删除（或者单元从`array`中删除）。

<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">操作成功是指对于那些不存在的或存在且可配置（configurable，参见《你不知道的JavaScript（上卷）》的第二部分第3章）的属性，`delete`返回`true`，否则返回`false`或者产生错误。</td>
    </tr>
</table>
...
`a = 42`中的`=`运算符看似没有副作用，实际上它的结果值是`42`，副作用是将`42`赋值给`a`。

<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/suggestion.png'></td>
        <td style="border: none;">组合赋值运算符，如`+=`和`-=`等，也同理。例如，`a = b += 2`首先执行`b += 2`（即`b = b + 2`），然后结果值再被赋给`a`。</td>
    </tr>
</table>

在多个赋值语句串联（链式赋值，chained assignment）的时候，赋值表达式（或语句）的结果值就能派上用场，比如：
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/warning.png'></td>
        <td style="border: none;">链式赋值常常被误用，例如：`var a = b = 42`，这看似和前面的例子差不多，实则不然。如果变量`b`没有在代码作用域中正式声明过，如`var b`，则`var a = b = 42`不会对变量`b`进行声明。这在严格模式中会产生错误，或者会无意中创建一个全局变量（参见《你不知道的JavaScript（上卷）》中的第一部分“作用域和闭包”）。</td>
    </tr>
</table>
...
上述代码可以运行，很多开发人员也喜欢这样操作。然而，其实可以通过赋值语句的副作用将两个`if`语句合二为一：
...
JavaScript语法规则中，有时同样的语法在不同的情况下会有不同的解释。孤立地理解这些规则会很吃力。

这里不一一列举，只介绍一些常见情况。

####1. 大括号

以下两种情况会用到大括号`{ .. }`（随着JavaScript的演进会出现更多类似情况）。
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">`a`是赋值的目标，称为“左值”（l-value）。`{ .. }`是所赋的值（即本例中赋给变量`a`的值），称为“右值”（r-value）。</td>
    </tr>
</table>
...
很多开发人员认为，此处的`{ .. }`只是一个孤立的`object`常量，并未被赋值。实际情况并非如此。

`{ .. }`在这里只是一个普通的代码块。这在JavaScript中并不多见（在其他语言中则常见得多），但在语法上是完全合法的，特别是和`let`（块作用域声明）一起使用时，它能派上很大用场（参见《你不知道的JavaScript（上卷）》中的第一部分“作用域和闭包”）。

`{ .. }`的作用和`for`/`while`循环以及`if`条件语句中代码块的作用基本相同。

但是`foo: bar()`这样奇怪的语法怎么也是合法的呢？

这里涉及JavaScript中一个不太为人知（也不建议使用）的特性，叫作“标签语句”（labeled statement）。`foo`是语句`bar()`的标签（后面没有`;`，请参见5.3节）。那么标签语句具体有何作用呢？

如果JavaScript有`goto`语句，理论上可以使用`goto foo`跳转到`foo`处执行。`goto`被普遍认为是一种十分糟糕的编码方式，它会让代码变得晦涩难懂（也叫作“spaghetti code”），好在JavaScript不支持`goto`。

然而，JavaScript通过标签跳转（labeled jump）能够实现一定程度的`goto`。`continue`和`break`语句都可以带一个标签，因此能够像`goto`那样进行跳转。例如：
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">`contine foo`并不是指“跳转到标签`foo`所在的位置继续执行”，而是“执行标签为`foo`循环的下一轮循环”。所以这里的`foo`并非`goto`。</td>
    </tr>
</table>

上例中，`continue`跳过了循环`3 1`，`continue foo`（带标签的循环跳转，labeled-loop jump）跳过了循环`1 1`和`2 2`。

带标签的循环跳转更大的用处在于，和`break __`一起可以实现从内层循环跳转到外层循环。没有它们的话，实现起来有时会非常麻烦：
...
上例中如果用不带标签的`break`，就可能需要用到一两个函数调用和共享作用域的变量等，这样代码会更难懂，此时带标签的`break`或许是更好的选择。
...
带标签的循环/代码块十分少见，也常被人嗤之以鼻，因此不建议使用。例如，循环跳转也可以通过函数调用来实现。不过在某些情况下，它们也能派上用场，这时请务必写明注释！

JSON被普遍认为是JavaScript语言的一个真子集，`{"a":42}`这样的JSON字符串会被当作合法的JavaScript代码（请注意JSON属性名必须使用双引号！）。实际并非如此！如果你在控制台中输入`{"a":42}`，系统就会报错。

这是因为标签不允许使用双引号，所以`"a"`并不是一个合法的标签，因此后面不能带`:`。

JSON的确是JavaScript语法的一个子集，但是JSON本身并不是合法的JavaScript语法。

这里存在一个十分普遍的误区，即如果通过`<script src=..>`标签加载JavaScript文件，其中只包含JSON数据（比如某个API返回的结果），那它就会被当作合法的JavaScript代码来解析，只不过无法被访问到。JSON-P（将JSON数据封装为函数调用，比如`foo({"a":42})`）通过将JSON数据传递给函数来获得访问权。

`{"a":42}`作为JSON值没有任何问题，但是在作为代码执行时会产生错误，因为它会被当作一个带有非法标签的语句块来执行。`foo({"a":42})`就没有问题，因为`{"a":42}`在这里是一个传递给`foo(..)`的对象常量。因此准确地说，**JSON-P能将JSON转换为合法的JavaScript语法**。

####2. 代码块

JavaScript中另一个棘手的问题（涉及强制类型转换，见第4章）是：
...
看似`+`运算符根据第一个操作数（`[]`或`{}`）的不同会产生不同的结果，实则不然。

第一行中，`{}`出现在`+`运算符表达式中，因此被当作一个值（空`object`）来处理。第4章曾提及，`[]`会被强制类型转换为`""`，而`{}`会被强制类型转换为`"[object Object]"`。

但在第二行中，`{}`被当作一个独立的空代码块（不执行任何操作）。代码块结尾不需要分号，所以这里不存在语法上的问题。最后`+ []`将`[]`**显式强制类型转换**（见第4章）为`number`值`0`。

####3. 对象解构

从ES6开始，`{ .. }`也可用于“解构赋值”（destructuring assignment，详情参见本系列的*ES6 & Beyond*一书），特别是`object`的解构。例如：
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">`{ a, b }`实际上是`{ a: a, b: b }`的简化版本，两种写法均可，只是`{ a, b }`更为简洁。</td>
    </tr>
</table>

此外，还可以使用`{ .. }`对函数的命名参数（named function argument）做对象解构（object destructuring），以方便从对象属性隐式地赋值：
...
`if (b) { .. } else { .. }`实际上是跟在`else`之后的一个单独语句，所以带不带`{ }`都行。换句话说，`else if`不符合前面说的编码规范，`else`后面实际上只是一个单独的`if`语句。
