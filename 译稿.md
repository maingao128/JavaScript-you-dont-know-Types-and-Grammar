...
![](images/JJ-??)
对数组中的值执行`delete`操作能够将其从数组中移除，需要注意的是，即使值被移除后，数组的`length`属性并不会相应更改。我们将在`第五章`中详细介绍`delete`操作。
...
![](images/JJ-??)
Array.from(..)有几个强大的功能，我们将在本系列的`《ES6及其他》`一书中详细介绍。
...
此外，很多`数组`中很有用的方法在`字符串`中并不支持，但是我们为`字符串`“借用”`数组`中那些不变更内容的方法：
...
我们再来看看另外一个例子，反转`字符串`（一个经常在JavaScript面试中遇到的问题！）。`数组`中有一个叫做`reverse()`增变方法，而`字符串`没有：
