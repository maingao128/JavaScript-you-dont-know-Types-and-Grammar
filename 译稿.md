...
`apply(..)`是一个工具函数，适用于任何函数对象，它用一种特殊的方式来调用传递给它的函数。

第一个参数是`this`对象（在本系列的<i>《this & Object Prototypes》</i>一书中有介绍），我们这里不用管，将它设置为`null`。第二个参数应该是一个`数组`或者类似数组的值（即“类数组对象”）。该“数组”中的值作为被调用函数的参数。
