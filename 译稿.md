...
`数组（array）`，`字符串（string）`，和`数字（number）`是程序的基本组成部分，然而在JavaScript中，这些类型的部分特性可以说是“让我欢喜让我忧”。下面我们来逐一了解以便能合理运用。

###数组

和强类型语言不同，在JavaScript中`数组`可以容纳任何类型的值，可以是`字符串`，`数字`，`对象（object）`，甚至是其他`数组`（多维数组就是通过这种方式来实现的）：
...
我们在创建“稀疏（sparse）”`数组`（含有空白／空缺单元的数组）时需要特别注意：
...
这里有一个坑需要多加注意，即如果`字符串`键值能够被强制类型转换为十进制`数字`的话会被当做`数字`索引来处理！
...
在`数组`中使用`字符串`作为键值/属性并不是一个好方法。建议使用`对象`来存放键值/属性这样的值，用`数组`来存放通过数字索引的值。

###类数组

有时我们需要将类`数组`（一组通过数字索引的值）转换为真正的`数组`，通常是对一组值调用`数组`工具函数（如：`indexOf(..)`，`concat(..)`，`forEach(..)`等）。

例如，一些DOM查询操作返回DOM元素列表，它们并非真正意义上的`数组`，但和`数组`十分类似。另一个常见的例子是一些函数通过`arguments`对象（类数组）以列表的方式来访问函数参数（在ES6中已废止）。
...
如上例所示，如果`slice()`不带其他参数的话会返回当前数组参数（在本例中是一个类`数组`）的一个复本。
...
`字符串`通常被看作是由字符组成的`数组`。然其内部实现有没有用到`数组`并不一定，重点在于我们要认识到JavaScript中的`字符串`和字符`数组`并不是一回事。它们仅仅是表面上看起来像而已。
...
如上例所示，`字符串`和`数组`确实有相似之处，都是类`数组`。比如它们都有`length`属性，以及`indexOf(..)`（从ES5开始`数组`支持此方法）和`concat(..)`方法：
...
JavaScript中的`字符串`是不可变（immutable）的，而`数组`是可变的。并且`a[1]`这样的做法在JavaScript中并非总是合法。老版本的IE就不允许（现在可以了）。*正确*的做法应该是`a.charAt(1)`。
...
很多`数组`函数在处理`字符串`时非常有用，虽然`字符串`没有这些函数，但我们可以从`数组`中“借用”这些非变更方法（nonmutation）来处理`字符串`：
...
另外一个例子是`字符串`的反转（JavaScript面试常见问题）。`数组`有一个`字符串`没有的可变更成员函数`reverse()`：
...
遗憾的是我们无法“借用”`数组`的可变更成员函数，因为`字符串`是不可变的：
...
一个变通（也叫破解）的办法是先将`字符串`转换为`数组`，处理完毕后再将结果转换回`字符串`：
...
对于简单的`字符串`这样完全没问题，有时这类简单粗暴的办法也挺管用。
...
如果我们经常需要以*字符数组*的方式来处理`字符串`的话，不如直接用`数组`。这样就不用在`字符串`和`数组`之间来回折腾了。可以在需要时用`join("")`将`字符数组`转换为`字符串`。

###数值

JavaScript中只有一种`数字`类型，包括“整数”和带小数的十进制数。“整数“之所以加引号是因为和其他语言不同，JavaScript中没有真正意义上的整数，这也是它一直以来为人所诟病的地方。这种情况在将来或许会有所改观，不过就目前而言所有的数值均为`数字`类型。

JavaScript中的“整数”就是没有小数位的十进制数。所以`42.0`即等同于“整数”`42`。

与大部分现代编程语言一样，包括所有的脚本语言，JavaScript的`数字`类型是基于“IEEE 754”标准来实现的，通常被称为“浮点数”。JavaScript使用的是其中的“双精度”格式（即“64位二进制”）。

网上很多文章详细介绍了二进制浮点数在内存中的存储方式，以及不同方式各自的考量。为了合理使用`数字`类型，我们并不一定要理解数位（bit）在内存中的存储方式，所以本书对此不多做介绍，有兴趣的读者可以参见IEEE 754的相关细节。
...
特别大和特别小的`数字`默认用指数格式显示，即`toExponential()`函数的输出结果，例如：
...
由于`数字`值可以使用`Number`对象来进行封装（boxed，见`第三章`），因此`数字`值可以调用`Number.prototype`中的方法（见`第三章`）。例如，`toFixed(..)`方法能够指定小数部分的显示位数：
...
请注意上例中的输出结果实际上是给定`数字`的`字符串`形式，如果指定小数部分的显示位数多于实际值的位数则用`0`补齐。
...
以上方法不仅适用于`数字`变量，也适用于`数字`常量。不过对`.`运算符我们需要特别注意，因为它也是一个合法的数字字符，会被优先识别为`数字`常量的一部分，然后才是对象属性访问运算符。
...
`42..toFixed(3)`则没有问题，因为第一个`.`被视为`数字`的一部分，第二个`.`是属性访问运算符。只是这样看起来有些奇怪，现实中也很少见。在基本类型值上直接调用方法的形式并不常见，不过不常见并不代表*不好*和*不对*。
...
然而对于`数字`常量而言这样的语法很容易造成误导，不建议使用。

`数字`还可以用指数格式（exponent form）表示，常用于较大的数字，如：
...
我们在处理带有小数的数字时需要特别小心。很多（或许是绝大多数）程序仅仅需要处理整数（integer），最大不超过百万或者万亿，此时JavaScript的`数字`类型是绝对可靠的。
...
`数字`的呈现方式决定了“整数”的安全值范围，其远小于`Number.MAX_VALUE`。
...
值得庆幸的是大数值操作并不常见（其比较操作可以通过`字符串`来实现）。如果我们确实需要对大数值进行数学运算的话，就目前来说还是需要借助相关的工具库。将来JavaScript也许会加入对大数值的支持。
...
虽然整数最大能够达到53位，但是部分数字操作（如：数位操作）只适用于32位`数字`，所以这些操作中`数字`的安全范围就要小很多。
...
`void`运算符在其他一些情况下也能派上用场，比如如果我们不想表达式返回任何结果，即使它有边缘效应（side effects）。
...
`数字`类型中有几个特殊的值，我们将在这里详细介绍。

#####不是数字的数字

如果数学运算中的操作数不是`数字`类型（或者不能被解析为常规的十进制或十六进制`number`），那么运算将无法返回一个合法的`数字`，此时返回值为`NaN`。
...
这说明“不是数字的数字”的类型仍然是`数字`。真是很绕啊！

`NaN`是一个“警戒值”（sentinel value，具有特殊意义的常规值），用来表示`数字`类型中的错误状态，意思是“试图执行一个数学运算但是没有成功，这是操作失败返回的`数字`”。
...
实际并没有这么简单。`isNaN(..)`有一个严重的缺陷。它的检查方式过于死板，即“检查传递进来的参数是否不是`NaN`，也不是`数字`”。但是这样做结果并不十分准确：
...
有意思的是如果反过来将其从`字符串`转换为`数字`，结果都是准确的：
...
有些应用程序中的数据需要用级数形式来表示（比如动画帧的移动速度），`数字`的符号位（sign）用来表示另外的信息（比如移动的方向）。这时如果一个包含`0`值的变量丢失了它的符号位，就会丢失其中包含的方向信息。所以保留`0`值的符号位可以防止这类情况发生。
...
例如，在C++中如果要向函数传递一个`数字`，它的值在函数中会被更改，可以这样声明参数`int& myNum`，向该函数传递变量`x`时，`myNum`即为指向`x`的引用。引用是一种特殊的指针，即指向变量的指针（*别名*）。如果不将参数声明为引用，则参数值*总是*通过值拷贝的方式传递，即使复杂的对象值也是如此。
...
我们将`a`传递给函数的时候，实际是将`a`的一个复本赋值给`x`，而`a`仍然指向`数组``[1,2,3]`。在函数中我们可以通过引用`x`来更改该`数组`的值（`push(4)`，更改后的新值为`[1,2,3,4]`）。但赋值语句`x = [4,5,6]`并不影响`a`指向的值，所以`a`仍然指向更改后的新值`[1,2,3,4]`。
...
如果要通过值拷贝的方式来传递复合值（compound value，比如`数组`），就需要为值创建一个拷贝，这样传递的就不再是原始值了。例如：
...
相反，如果要将标量基本类型值传递到函数内并进行更改，就需要将该值封装到一个复合值（`对象`，`数组`，等）中，然后通过引用拷贝（reference-copy）的方式传递。
...
如果需要传递指向标量基本类型值的引用，比如`2`，则可以将其封装到对应的`数字`封装对象中（见`第三章`）。

虽然这里传递的是一个指向`数字`对象的引用拷贝，但和预想的不同，我们并不能通过它来更改其中的基本类型值：
...
当执行`x = x + 1;`时，`x`中的标量基本类型值`2`被从`数字`对象中拆封（unboxed，或者提取，extracted）出来，`x`神不知鬼不觉地从引用变成了`数字`对象，它的值为`2 + 1`等于`3`。然而函数外的`b`仍然指向原来那个值为`2`的`数字`对象。
...
JavaScript中的`数组`是通过数字索引的一组任意类型的值，`字符串`和`数组`类似，但是它们的行为特征不同，在将`字符串`作为`数组`处理时需要特别小心。JavaScript中的`数字`包括“整型”和“浮点型”。
...
简单标量基本类型值（`字符串`，`数字`等）通过值拷贝来赋值／传递，复合值（`对象`等）通过引用拷贝来赋值／传递。这里的引用和其它语言中的引用／指针不同，它们不能指向别的引用／指针，只能指向值。
