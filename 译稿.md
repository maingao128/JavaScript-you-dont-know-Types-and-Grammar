...
值得注意的是`typeof`显示的不是这些对象本身的*类型*，而是`对象`类型的子类型。
...
`typeof`返回`"object"`的值（如`数组`）包含一个额外的内部属性`[[Class]]`（你可以把它看作是一个内部的分类，而非传统意义上的类）。我们无法直接访问这个属性，一般都是通过`Object.prototype.toString(..)`来访问它。例如：
...
如果你需要经常访问这些`字符串`中的属性和方法，比如在`for`循环中使用`i < a.length`，或许从一开始就创建一个封装对象会比较好，这样JS引擎就不需要每次自动为你创建封装对象了。
...
对于`数组`，`对象`，`函数`，和`正则表达式`来说，我们一般倾向通过常量的方式来创建值，实际上使用常量和使用构造函数的效果是一样的（创建的值都通过封装对象来包装）。
...
我们可以看到，`join(..)`首先假定数组不为空，然后通过`length`属性值来遍历其中的元素。而`map(..)`并不做这样的假定，所以它对“空单元（empty slots）”`数组`的处理结果出乎我们的预料，也容易导致各种问题。
...
第一个参数是`this`对象（在本系列的<i>《this & Object Prototypes》</i>一书中有介绍），我们这里不用管，将其设为`null`。第二个参数应该是一个`数组`或者类似数组的值（即“类数组对象”）。该“数组”中的值作为被调用函数的参数。
...
例如，通过封装（boxing）对`字符串`基本类型进行扩展后，所有字符串对象都能够访问`String.prototype`对象中定义的方法。
...
对于变量`b`而言，强制类型转换是隐式的，由于`+`操作符有一个操作数是`字符串`，所以这实际是`字符串`拼接操作，作为它的一个隐式边缘效应，数字`42`被强制类型转换为相应的字符串值：`"42"`。

函数`String(..)`则是显式地将`a`强制类型转换为`字符串`。
...
在理解了`a + ""`的前提下，如果你有意识地以这样的方式来进行`字符串`转换，可能觉得它是“显式”的。反之如果你根本不知道`String(..)`能够被用于`字符串`强制类型转换，就可能会觉得它是“隐式”的。
...
在介绍“显式”和“隐式”强制类型转换之前，我们需要掌握值转换的基本规则（将值转换为`字符串`，`数字`，和`布尔型`）。ES5规范第9节中定义了一些“抽象操作”（“仅内部使用的操作”的规范用语）及其转换规则。我们这里着重介绍`ToString`，`ToNumber`，和`ToBoolean`，附带讲一讲`ToPrimitive`。

####ToString

规范第9.8节中定义了抽象操作`ToString`，它负责从非字符串值到`字符串`值的强制类型转换。

内建基本类型值有自己的字符串化操作（stringification）：`null`转换为`"null"`，`undefined`转换为`"undefined"`，`true`转换为`"true"`。`数字`的字符串化遵循我们熟知的规则，但是`第二章`中介绍过那些非常小和非常大的`数字`使用的是指数形式：
...
但是之前我们介绍过，如果对象有自定义的`toString()`方法，在其转换为`字符串`时该方法会被调用，得到的结果`字符串`即是它的返回值。
...
`数组`有一个经过重载的缺省`toString()`方法，将所有单元的值字符串化后通过`","`连接起来：
...
在使用工具函数`JSON.stringify(..)`将JSON对象序列化为`字符串`时也涉及到`ToString`。

请特别注意，这里的字符串转换并非严格意义上的强制类型转换。不过由于涉及前面提到的`ToString`规则，我们就此岔开话题介绍一下JSON字符串化。

对于大部分简单值来说，JSON字符串化基本和`toString()`的效果一样，只不过序列化总是返回一个`字符串`：
...
`JSON.stringify(..)`工具在遇到`undefined`，`function`，和`symbol`时会返回`undefined`，在`数组`中会用`null`替代（从而保证单元位置索引不变），如果是对象属性则会被忽略。
...
我们通常会误以为`toJSON()`方法必须返回一个经过JSON字符串化的值。这并不准确，除非我们确实需要字符串化`字符串`（通常不会！）.`toJSON()`应该返回一个适当的值（可以是任何类型），然后再由`JSON.stringify(..)`对其进行字符串化。
...
在上例的第二个函数中，我们是对`toJSON`返回的`字符串`做字符串化，而非`数组`本身，这可能并非是我们想要的结果。
...
如果`replacer`是一个`数组`，那么它必须是一个`字符串``数组`，其中包含序列化要处理的对象的属性名称。`数组`之外的属性被忽略。

如果`replacer`是一个`函数`，它会对`对象`本身调用一次，然后对`对象`的每个属性调用一次，每次调用传递两个参数，`key`和`value`。如果要在序列化中忽略某个`key`，则返回`undefined`，否则返回指定的*值*。
...
`JSON.string`还可以带第三个参数，即可选参数*space*，它用来指定人性化输出的缩进。*space*值为正整数时是指定每一级缩进的字符数，该值还可以是`字符串`，此时最前面的10个字符会被用来作为每一级的缩进：
...
2. 如果向`JSON.stringify(..)`函数传递定义了`toJSON()`方法的`对象`，那么`toJSON()`方法会在字符串化之前被调用，将对象“强制类型转换”（类似于）为*安全的JSON值*。

####ToNumber

在有些情况下非`数字`值需要被当作`数字`来使用，比如数学运算。为此ES5规范在第9.3节定义了抽象操作`ToNumber`。

`true`转换为`1`，`false`转换为`0`。`undefined`转换为`NaN`，`null`转换为`0`。

`ToNumber`对`字符串`值的处理基本遵照`数字`常量相关规则／语法（见`第三章`）。处理失败则返回`NaN`（在处理`数字`常量失败时会产生语法错误）。另外`ToNumber`对以`0`开头的十六进制数并不按十六进制处理（而是按十进制，见`第二章`）。
...
对象（以及数组）会首先被转换为相应的基本类型值，然后（返回结果为基本类型，但还不是`数字`）再根据上述规则被强制转换为`数字`。
...
答案是`true`。虽然它们看起来象是`假值`，实际上`字符串`都是`真值`，除了`""`，它是`假值`列表中唯一的一个`字符串`。
...
让我们从最常见的`字符串`和`数字`之间的强制类型转换开始。

`字符串`和`数字`之间的转换使用的是`String(..)`和`Number(..)`这两个内建函数（我们称为“原生构造函数”，见`第三章`），值得注意的是它们前面没有`new`关键字，因此并不创建封装对象。
...
`String(..)`遵循前面介绍的`ToString`规则，将其他类型的值转换为`字符串`。`Number(..)`遵循前面介绍的`ToNumber`规则，将其他类型的值转换为`数字`。
...
除了`String(..)`和`Number(..)`外，还有其他方法可以实现`字符串`和`数字`之间的转换：
...
上例中`+c`是`+`操作符的*一元（unary，即只有一个操作数）*形式。`+`操作符显式地将操作数`c`转换为`数字`，而非对其进行数学加法运算（也不是字符串拼接操作－－见下）。

那么`+c`是不是显式强制类型转换呢？这取决于我们自己的经验和理解。如果我们已经知道一元运算符`+`会将操作数显式强制类型转换为`数字`，那么它就是显式的。如果我们不知道的话，它就是令人困惑的、有隐藏边缘效应的隐式强制类型转换。
...
一元运算符`+`的另一个常见用途是将`日期`对象强制类型转换为`数字`，因为其结果值是Unix时间戳格式，以微秒为单位（从1970年1月1日00:00:00 UTC起到现在的时间）：
...
当字位操作符（如`|`和`~`）和某些特殊`数字`一起使用时会产生强制类型转换的效果，从而导致结果值为另一个`数字`。
...
以`-(x+1)`为例。唯一能得到结果`0`（或者严格来说是`-0`）的`x`是哪个值？答案是`-1`。换句话说，如果输入值为`-1`，`~`和一组`数字`一起能够得到一个假值`0`（能够被很简便地强制类型转换为`false`），否则则得到其他的真值`数字`。

这和我们的讨论内容有什么关系？

`-1`通常被称为“哨位值”，意指一个值在其同类型值（`数字`）中被赋予了特殊的含义。在C语言中我们用返回值`-1`来表示函数执行失败，用大于等于`0`的返回值来表示函数执行成功。

JavaScript在定义`字符串`操作`indexOf(..)`时也遵循此惯例，该操作在给定字符串中搜索一个子字符串，如果找到则返回子字符串的位置（从0开始），否则返回`-1`。

通常我们不仅使用`indexOf(..)`来获得子字符串的位置，也用它来检查`字符串`是否包含子字符串，相当于一个`boolean`判断。例如：
...
有些程序员会使用`~~`来截除`数字`值的小数部分（例如将其转换为整数），他们觉得这与`Math.floor(..)`的效果一样，实际上并非如此。
...
解析`字符串`中的`数字`也能够达到将`字符串`强制类型转换为`数字`的效果。但这和我们之前介绍过的方法还是有很大区别。
...
解析`字符串`中的`数字`时允许字符串含有非数字字符，按从左自右的顺序，如果遇到非数字字符则解析停止。然而强制类型转换不允许非数字字符，否则会失败并返回`NaN`。

字符串解析和强制类型转换之间并非替代关系。它们虽然相似，但用途不同。如果字符串右边的非数字字符可以被忽略，那么使用字符串解析没有问题。而将`字符串`强制类型转换为`数字`则要求字符串中所有的字符都是数字，象"42px"这样的非数字字符串是不行的。
...
非字符串参数会首先被强制类型转换为`字符串`（见`第59页上的“ToString”`），显然这是*隐式*强制类型转换，并非很好的编码风格，所以应该避免向`parseInt(..)`传递非字符串参数。
...
首先，上例中的第一个错误是向`parseInt(..)`传递了一个非字符串，这完全是在自找麻烦。这时JS会将参数强制类型转换为它能够处理的`字符串`。
...
就象一元运算符`+`将值强制类型转换为数字（见前面的介绍），一元运算符`!`显式地将值强制类型转换为`boolean`。*问题*是它同时还将正值反转为负值（或者反之）。所以JS程序员经常使用`!!`操作符将值显式强制类型转换为布尔值，因为第二个`!`会将结果反转为原值：
...
让我们从另一个角度来看看*隐式*强制类型转换是怎么回事，应该如何使用，而不是仅仅将其看作“显式强制类型转换的对立面”，这样理解太狭隘，忽略了它们之间的一个重要的差别。
...
操作符边缘效应的另一个例子是`delete`。我们在`第二章`中介绍过，我们用`delete`来删除`对象`的属性，或者`数组`的单元。通常它是以独立语句的形式出现：
...
如果操作有效／允许，`delete`返回结果`true`，否则返回`false`。它的边缘效应则是属性被从`对象`中删除（或者单元被从`数组`中删除）。
