...
我们有时会误以为可以使用括号`( )`将`a++`的边缘效应封装起来，例如：
...
可惜不是这样，`( )`本身并不是一个封装表达式，不会在表达式`a++`产生边缘效应之后执行。实际上即便如我们所想，`a++`首先返回`42`，除非有表达式在`++`之后再次对`a`进行运算，否则还是不会得到`43`，`b`也不会被赋值为`43`。

不过还是有一个办法，即`,`语句系列逗号运算符。它能够让我们将多个独立的表达式语句串联成一个语句：
...
运算符边缘效应的另一个例子是`delete`。我们在`第二章`中介绍过，用`delete`来删除`object`的属性，或者`array`的单元。通常它是以独立语句的形式出现：
