...
这或许和我们想的不一样，即`<=`应该是“小于或者等于”。JS中`<=`是指“不大于”（即`!(a > b)`，处理为`!(b < a)`）。`a >= b`处理为`b <= a`，遵循相同的规则。

相等比较中有严格相等，而关系比较中没有“严格关系比较（strict relational comparison）”。也就是说在关系比较中的隐式强制类型转换是无法避免的，比如`a < b`，我们只能确保`a`和`b`在比较中类型一致，除此之外别无他法。
