...
这是怎么回事？我们都知道`"42"`是一个真值（见本章前面的介绍）。为什么`==`的结果不是`true`呢？原因即简单又复杂，很容易让人掉坑里，很多JS程序员对此并未引起足够的重视。
...
也就是说，字符串`"42"`既不`== true`，也不`== false`。看起来似乎有点奇怪，一个值怎么可以既非真值也非假值呢？

这个问题本身就是错误的，一切都是我们的大脑惹的祸。

因为，`"42"`的确是一个真值，但是`"42" == true`并非在进行`boolean`值的比较和强制类型转换，和你所想的不一样。`"42"`没有被强制类型转换为`boolean`（`true`），而是`true`被强制类型转换为`1`，`"42"`被转换为`42`。
