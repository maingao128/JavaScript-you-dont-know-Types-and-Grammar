...
对于变量`b`而言，强制类型转换是隐式的，由于`+`运算符有一个操作数是`string`，所以这实际是`string`拼接操作，作为它的一个隐式副作用，数字`42`被强制类型转换为相应的字符串值：`"42"`。
...
我们可以向`JSON.stringify(..)`传递第二个参数，即可选参数（optional argument）*replacer*。它可以是`array`或者`function`，通过某种过滤机制来指定哪些属性应该被处理和排除，然后对结果进行序列化，以此来实现对象递归序列化的定制，类似`toJSON()`。

如果`replacer`是一个`array`，那么它必须是一个`string``array`，其中包含序列化要处理的对象的属性名称。`array`之外的属性被忽略。

如果`replacer`是一个`function`，它会对`object`本身调用一次，然后对`object`的每个属性调用一次，每次调用传递两个参数，`key`和`value`。如果要在序列化中忽略某个`key`，则返回`undefined`，否则返回指定的*值*。
...
那么`+c`是不是显式强制类型转换呢？这取决于我们自己的经验和理解。如果我们已经知道一元运算符`+`会将操作数显式强制类型转换为`number`，那么它就是显式的。如果我们不知道的话，它就是令人困惑的、有隐藏副作用的隐式强制类型转换。
...
*隐式*强制类型转换指的是那些隐蔽的类型转换，其副作用（side effects）也不是很明显。换而言之，任何你觉得不明显的类型转换都是*隐式*类型转换。
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">之所以说<b>a || b</b>大致等价于（roughly equivalent）<b>a ? a : b</b>是因为它们的返回结果相同，但是它们之间存在一个细微的差别。在<b>a ? a : b</b>中，如果<b>a</b>是更为复杂一些的表达式（比如：一个有副作用的函数调用，等等），则<b>a</b>表达式有可能被执行两次（如果第一次结果为真）。相反，对于<b>a || b</b>，表达式只会执行一次，其结果值会被用于条件判断以及返回值（如果适用的话）。<b>a &amp;&amp; b</b>和<b>a ? b : a</b>也一样。</td>
    </tr>
</table>
...
如果让`a.valueOf()`在每次调用时都产生副作用，如第一次调用时返回`2`，第二次调用时返回`3`，就会出现这样的情况。很简单：
...
我们在介绍`ToNumber`时说过，右边的值`[2]`和`[null]`会执行`ToPrimitive`强制类型转换，以便能够和左边的基本类型值进行比较（`2`和`""`）。因为`array`的`valueOf()`返回`array`本身，所以强制类型转换将对`array`进行字符串化。
...
*隐式*强制类型转换则不那么明显，只是伴随其他操作的副作用发生。它看似*显式*强制类型转换的反面，让人感觉很糟糕，实际上*隐式*强制类型转换也有助于提高代码的可读性。
...
表达式`a + 3`本身没有副作用，如改变`a`的值等。它的结果值为`5`，其通过`b = a + 3`被赋给变量`b`。

最常见的（有可能）有副作用的表达式是函数调用：
...
很多人误以为变量`b`和`a`的值都是`43`。这是因为没有完全理解`++`运算符的副作用产生的时机。
...
`++`在操作数前面时，如：`++a`，它的副作用（递增`a`）发生在表达式返回结果值之前，而`a++`的副作用发生在之后。

<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">考考你，<b>++a++</b>这样的语法是否合法？你试一试就知道，它会产生<b>ReferenceError</b>错误。因为运算符需要将它产生的副作用赋值给一个变量。以<b>++a++</b>为例，首先执行<b>a++</b>（根据运算符优先级，见下），返回<b>a</b>递增之前的值<b>42</b>。然后执行<b>++42</b>，这时会产生<b>ReferenceError</b>错误，因为<b>++</b>无法直接在<b>42</b>这样的值上产生副作用。</td>
    </tr>
</table>

我们常会误以为可以用括号`( )`将`a++`的副作用封装起来，例如：
...
可惜并非如此，`( )`本身并不是一个封装表达式，不会在表达式`a++`产生副作用之后执行。实际上即便如我们所想，`a++`首先返回`42`，除非有表达式在`++`之后再次对`a`进行运算，否则还是不会得到`43`，`b`也不会被赋值为`43`。
...
如果操作成功，`delete`返回`true`，否则返回`false`。它的副作用则是属性从`object`中删除（或者单元从`array`中删除）。
...
`a = 42`中的`=`看似没有副作用。但是如果我们检查一下语句`a = 42`的结果值就会发现，它返回的是`42`，所以`=`的副作用就是给`a`赋值。

<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/suggestion.png'></td>
        <td style="border: none;">副作用同样适用于+=和-=这样的组合赋值运算符。例如，<b>a = b += 2</b>首先执行<b>b += 2</b>（即<b>b = b + 2</b>），该操作的结果值被赋给<b>a</b>。</td>
    </tr>
</table>
...
这里`c = 42`的结果值为`42`（产生的副作用是`42`被赋值给`c`），然后`b = 42`的结果值为`42`（产生的副作用是`42`被赋值给`b`），最后是`a = 42`（产生的副作用是`42`被赋值给`a`）。
...
很多开发人员喜欢这样做，这完全没问题。但是我们可以利用赋值语句的副作用将两个`if`语句合二为一：
...
因为`,`运算符的优先级比`=`低。所以`b = a++, a`是作为`(b = a++), a`来执行。因为之前介绍过`a++`有*后续副作用（after side effects）*，所以赋给`b`的值是`++`对`a`做递增前的值`42`。
...
语句（statements）和表达式（expressions）在英语中有对应的称谓－语句就象句子（sentences），而表达式就象短句（phrases）。表达式可以是简单独立的，也可能产生副作用。
