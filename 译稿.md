...
上面这些特殊数字无法以32位方式呈现（因为它们来自于64位IEEE 754标准，见`第二章`），因此`ToInt32`对它们返回`0`。

至于`0 | ___`是`显式`还是`隐式`则存在着争议。从规范的角度来说毫无疑问是`显式`的，但是如果你对字位操作符没有足够的理解，它对你可能就是`隐式`的魔术。不过为了和本书其他地方保持一致，我们将其视为`显式`。
