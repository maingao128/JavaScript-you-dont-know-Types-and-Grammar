...
很多程序员喜欢这样做，这完全没问题。但是我们可以利用赋值语句的边缘效应将两个`if`语句合二为一：
...
带标签的循环／代码块十分少见，and often frowned upon(JJ--??)。所以可能的话尽量不要使用它们。例如，可以在循环中使用函数来实现跳出。但是它们在某些特殊的情况下可能会有帮助。在使用标签实现跳转时请务必使用批注写清楚你要实现的功能！
