...
运算符`&&`和`||`首先结合，然后是`? :`。但如果出现相同优先级的多个运算符时怎么办？它们是否按从左到右或者从右到左的顺序执行？
...
但是仍然有一个重要的问题：我们是否只需根据对操作数优先级和关联性的理解来编码，无需考虑易读性？或者需要使用`( )`来人为地控制结合和执行顺序？

或者从另一个角度，虽然这些规则通过学习是可以掌握的，但是其中也有很多gotcha(JJ--??)，我们是否应该尽量避免依赖这些优先级和结合规则本身？如果答案是肯定的，我们就需要人为地使用`( )`来控制组合，而非依赖规则本身？
...
JavaScript详细定义了运算符的优先级（结合运算符的先后顺序）和关联性（表达式中的多个运算符如何组合）。一旦你掌握了这些规则，你就能够自己判断是否使用它们来让代码更佳简洁明了，或者因为它们过于隐秘而应避免使用。
