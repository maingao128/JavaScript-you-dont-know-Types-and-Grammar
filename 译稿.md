...
本书使用了“JS”一词，这并非贬义，虽然作为程序员我们最喜欢的就是吐槽编程语言。

从互联网最早期开始，JavaScript就一直是实现交互体验的基本技术。虽然最初是用来实现诸如鼠标移动轨迹闪烁效果，以及烦人的弹出消息框这些东西。近二十年之后，JavaScript技术和功能得到了数量级的提升，没有人再质疑它在互联网这个世界上最广为使用的软件平台中的重要地位。
...
JavaScript有七种内置类型：`null`，`undefined`，`boolean`，`number`，`string`，`object`，和`symbol`。可以使用`typeof`操作符来标志。
...
这些方法不仅适用于`number`变量，也适用于`number`常量。不过在使用`.`操作符时需要特别注意，因为它是一个合法的数字字符，它会被尽可能优先识别为`number`常量的一部分，然后才是对象属性访问操作符。
...
`undefined`是一个内置的标志符（除非被修改，见前面的介绍），其值为`undefined`，要得到该值还有另外一个方法，即通过`void`操作符。
...
`void`操作符在其他一些情况下也非常有用，比如你需要确保表达式不返回任何结果时，即使它有边缘效应（side effects）。
...
在某些特殊的应用程序中程序员需要使用级数形式来表示数据（如动画帧的移动速度），用`number`的符号来表示其他一些信息（如移动的方向）。
...
复合值，既`object（对象）`（包括`array`，所有的封装对象，见`第三章`）和`function（方法）`，*总是*通过引用拷贝的方式来赋值／传递。
...
虽然标志事实上并非私有（private）（用`Object.getOwnPropertySymbols(..)`可以得到对象中的所有标志），但它主要的应用范围还是私有和特殊属性。很多程序员用它来替换那些名称中含有下划线（_）前缀的属性，下划线前缀通常用于命名私有和特殊属性。
...
对于变量`b`而言，强制类型转换是隐式的，由于`+`操作符有一个操作数是`string`，所以这实际是`string`拼接操作，作为它的一个隐式边缘效应，数字`42`被强制类型转换为相应的字符串值：`"42"`。
...
请记住一点：我们编写的代码大都是需要给别人看的。即便你已经是一个JS专家，也需要顾及到其他水平层次的程序员，考虑他们是否能读懂你的代码，以及别人对于“显式”和“隐式”的定义是否和你一致？
...
那为什么它要是假值（`falsy`）呢？因为我们经常用将`document.all`强制类型转换为`boolean`（比如在`if`语句中）的方法来检测浏览器是否是老版本的非IE浏览器。IE自诞生以来就一直遵循浏览器标准，相比其他浏览器更加有力地推动了Web的发展。
...
这并不是一个好办法。对于这个问题许多JS程序员并不太了解，但如果我们对其放任自流只会让情况变得更糟。
...
显式强制类型转换是明确显示的类型转换，很多类型转换都属于此类。

我们力图将代码中的类型转换表达清楚，以免给别的程序员留坑。代码越清晰可读性越高。
...
上例中`+c`是`+`操作符的*一元（unary，即只有一个操作数）*形式。`+`操作符显式地将操作数`c`转换为`number`，而非对其进行数学加法运算（也不是字符串拼接操作－－见下）。
...
一元运算符`-`和`+`类似，不过它还会将数字的符号位进行反转（flip）。我们不能使用两个`-`（--）来实现撤销反转，因为`--`会被当作递减操作符来处理。所以我们需要使用`- -"3.14"`这样的形式，在中间加一个空格，这样结果才是`3.14`。
...
我们应该极力避免一元运算符`+`（和`-`）和其他操作符相邻。上面的例子运行起来没问题，但并不是一个很好的做法。另外`d = +c`（和`d =+ c`）也很容易与`d += c`混淆起来，它们之间的区别大了去了。
...
`~`操作符（即字位操作“非”）也涉及强制类型转换，它令人费解，又常常被忽视。以至于理解它的程序员也对其避而远之。秉承本书的一贯宗旨，让我们来深入探讨一下`~`有哪些用途。

在`第23页，“32位有符号整数”`一节中，我们说过字位操作符只适用于32位整数，即操作数会被强制使用32位表示。这是通过`ToInt32`抽象操作来实现的（ES5规范第9.5节）。

`ToInt32`首先进行`ToNumber`强制类型转换，如`"123"`将会首先被转换为`123`，然后再执行`ToInt32`。

当字位操作符（如`|`和`~`）和某些特殊`number`一起使用时会产生强制类型转换的效果，从而导致结果值为另一个`number`。

让我们先来看看`|`操作符（字位操作“或”）在空操作（no-op）`0 | x`中的情况，这时它仅执行`ToInt32`转换（在`第二章`介绍过）：
...
至于`0 | ___`是`显式`还是`隐式`还有争议。根据规范，这毫无疑问是`显式`的，但如果对字位操作符没有理解到一定程度，它可能就是`隐式`的。考虑到本书的一致性，我们将其视为*显式*的。
...
我们为什么要特别介绍字位反转，这个主题太晦涩，JS程序员也很少需要处理到字位级别。
...
有些程序员会使用`~~`来截除`number`值的小数部分（例如将其转换为整数），他们觉得这与`Math.floor(..)`的效果一样，实际上并非如此。
...
那我们为什么倾向于用`~~x`而不是`x | 0`呢？这是出于操作符优先级（详见`第五章`）的考虑：
...
就象一元运算符`+`将值强制类型转换为数字（见前面的介绍），一元运算符`!`显式地将值强制类型转换为`boolean`。*问题*是它同时还将正值反转为负值（或者反之）。所以JS程序员经常使用`!!`操作符将值显式强制类型转换为`boolean`值，因为第二个`!`会将结果反转为原值：
...
三元操作符`? :`判断条件是否为真，如果为真则将变量b赋值`true`，否则赋值`false`。
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;"><i>《JavaScript: The Good Parts》</i>的作者Douglas Crockford在许多场合和文章中都主张尽量不要使用JavaScript强制类型转换。在他看来<i>隐式</i>强制类型转换是很糟糕的。然而如果我们看看他的代码就会发现，他也使用了许多<i>隐式</i>和<i>显式</i>强制类型转换。事实上他的抱怨大都来自于<b>==</b>操作符，但在本章中你会发现这仅仅是强制类型转换的冰山一角。</td>
    </tr>
</table>
...
许多程序员认为如果某个机制能够带来好处*A*但是同时又有坏处*Z*，为了保险起见就干脆别碰。

我不同意这种“因噎废食”的想法。不要因为你看到的都是负面影响而想当然地认为*隐式*强制类型转换一无是处。它也有好的方面，希望越来越多的人能够发现并接受它。

#####隐式：字符串 <--> 数字

之前我们介绍了`string`和`number`之间的*显示*强制类型转换。现在让我们来看看它们之间的*隐式*强制类型转换。不过开始之前我们需要了解一些导致*隐式*强制类型转换的操作。

操作符`+`被重载以便同时支持`number`的加法和`string`的拼接。那么JS如何知道我们要执行哪种操作呢？例如：
...
是什么不同导致了`"420"`和`42`这两个不同的结果？常见的误解是因为某个或者两个操作数为`string`，因而`+`进行的是`string`拼接。这样的理解部分正确，实际情况比这个复杂得多。
...
两个操作数都不是`string`，但是它们均被强制转换为`string`然后进行拼接。这到底是怎么回事呢？
...
根据ES5规范，第11.6.1节，如果运算符是`string`，或者其能够根据下面步骤产生`string`表示的话，`+`运算（操作数为某个`object`值）将进行拼接操作。因此当`+`的其中一个操作数为`object`（包括`array`）的话，它首先对操作数的值调用`ToPrimitive`抽象操作（第9.1节），该抽象操作接着调用`[[DefaultValue]]`算法（第8.12.8节），上下文提示为`number`(JJ-??)。

如果你多加留意，会发现该操作和`ToNumber`抽象操作处理`object`的方式一样（`见第65页“ToNumber”`）。因为`array`的`valueOf()`操作无法产生简单基本类型，所以它将转而调用`toString()`产生`string`表示。因此上例中的两个数组变成了`"1,2"`和`"3,4"`。`+`将这两个`string`拼接起来产生你要的结果：`"1,23,4"`。

让我们暂时撇开这些繁琐的细节，回到之前那个简单的描述：如果`+`操作数其中一个是`string`（或者通过以上步骤可以产生），则执行`string`拼接操作。否则执行`number`加法。
...
意味着你可以通过将`number`和空字符串`""`相加来将其强制类型转换为字符串：
...
我们比较一下`a + ""`和之前介绍过的`String(a)`，两者之间有一个细微的区别需要注意。`a + ""`中，`ToPrimitive`抽象操作会对`a`调用`valueOf()`，返回值则通过`ToString`抽象操作转换为`string`。而`String(a)`则是直接调用`ToString()`。

两者最终都返回一个`string`，但是如果你使用的是`object`而非`number`基本类型，可能得到的结果会不一样！
...
操作符`-`只能用于数字减法，因此`a - 0`会将`a`强制类型转换为`number`。更为少见的，`a * 1`和`a / 1`也能得到相同的结果，因为这两个操作符也只能用于数字。

那么针对`object`执行`-`操作又如何？和前面的`+`类似：
...
上面两个数组都必须被转换为`number`，但它们首先被强制类型转换为`string`（使用`toString()`），然后被强制类型转换为`number`，以便`-`减法操作符能够执行。
...
我认为在某一种情况下，隐式强制类型转换会非常有帮助，就是将一些复杂的`boolean`逻辑转换为数字加法的时候。当然，这种方式并不常见，但是特殊情况需要特殊处理。
...
这里我们则可以使用从`boolean`到`数字（0或者1）`的强制类型转换：
...
首先我们用`!!arguments[i]`将值强制类型转换为`true`或`false`。因此你可以传递非`boolean`值，如：`onlyOne("42, 0)`，这样仍然有效（否则会返回拼接后的`string`，这样逻辑就不对了）。

一旦确定这是一个`boolean`，我们使用`Number(..)`进行另一个显式强制类型转换以确保返回值为`0`或者`1`。
...
现在让我们来看看到`boolean`的隐式强制类型转换，到目前为止，这类情况最为常见，也最容易出错。

请记住，这类转换涉及的是隐式强制类型转换。对于`number`和`string`操作来说，强制类型转换的过程很清楚。

那么，到`boolean`的隐式强制类型转换涉及哪些表达式和操作呢？
...
以上情况下的值，如果本身不是`boolean`的话，会被隐式强制类型转换为`boolean`，遵循本章之前介绍过的`ToBoolean`抽象操作的规则。
...
在以各种情况下，非`boolean`值会被隐式强制类型转换为`boolean`值，以便进行条件判断。
...
原因是在JavaScript中，它们返回的结果值并非是`boolean`，这与其他语言不同。

那么它们返回什么样的结果值呢？它们返回其两个操作数中的一个（且仅一个）的值。换句话说，它们选择操作数中的某一个，然后返回其值。

这里我们引述ES5规范中的第11.11节：

*&&和||操作符的返回值并不一定是`boolean`值。它们的返回值总是两个操作数其中的一个的值*
...
`||`和`&&`操作符首先会对第一个操作数（`a`和`c`）进行`boolean`条件判断。如果该操作数不是`boolean`值（如本例）则执行`ToBoolean`强制类型转换，以便执行条件判断。
...
没关系！不用太担心。你的代码没问题（可能）。问题只是你一直以来都不知道，复杂的条件表达式执行之后还有一个`boolean`值的隐式强制类型转换。
...
上面的代码仍然可以正常工作，除了一个细微的地方。`a && (b || c)`表达式的返回值实际上是`"foo"`，而不是`true`。`if`表达式将`foo`强制类型转换为`boolean`，结果为`true`。
...
ES6中的symbols引入了一个gotcha（JJ--??），这里我们需要简单介绍一下。处于某些超出本书讨论范围的原因，`symbol`到`string`的显式强制类型转换是允许的，然而对应的隐式强制类型转换却不行，会产生`错误`。
...
`Symbol`的值不能被强制类型转换为`number`（也会产生错误），但是可以被强制类型转换为`boolean`（`显式`和`隐式`，结果总是`true`），这很奇怪。
...
段落11.9.3.1的最后规定了针对`object`（包括`函数`和`array`）的宽松相等`==`。仅当两个对象指向同一值时才相等。这里不涉及强制类型转换。
...
为了阐明==的强制类型转换，让我们先来看看本章之前介绍过的`string`和`number`的例子：
...
那么这里进行的是什么样的强制类型转换呢？是`a`由数字`42`转换为`string`，还是`b`由字符串`"42"`转换为`number`？
...
规范中明确规定字符串值`"42"`被强制类型转换为`number`以便进行相等比较。前面我们已经介绍过强制类型转换的规则，及其涉及的`ToNumber`抽象操作。所以本例中两个值`42`是相等的。
...
2. 如果Type(y)是`boolean`值，Type(y)是数字，则返回x == ToNumber(y)的结果。
```
...
`Type(x)`是`boolean`值类型，所以执行`ToNumber(x)`将`true`强制类型转换为`1`。现在比较的是`1 == "42"`。它们的类型仍然不同，根据规则将`"42"`强制类型转换为`42`，最后`1 == 42`结果为`false`。
...
`Type(y)`是`boolean`值类型，所以执行`ToNumber(y)`将`false`强制类型转换为`0`。`"42" == 0`进而变成`42 == 0`，所以结果为`false`。

也就是说，字符串值`"42"`既不`== true`，也不`== false`。乍看起来，一个值既不是真值也不是假值，这似乎有点奇怪。

但是问题就在这里！这个问题本身就是完全错误的。错不在你，是你的大脑欺骗了你。

`"42"`的确是一个真值，但是`"42" == true`并非在进行`boolean`值的比较和强制类型转换，和你所想的不一样。`"42"`没有被强制类型转换为`boolean`（`true`），而是`true`被强制类型转换为`1`，`"42"`被转换为`42`。

这里并没有涉及`ToBoolean`，所以`"42"`是真值还是假值和`==`操作毫不相关！

这里我们需要理解`==`算法如何处理不同值类型的组合。它会先将`==`两边的`boolean`值强制类型转换为`number`。
...
如果你在代码中避免使用`== true`和`== false`（也叫做`boolean`值的宽松相等），你就不会担心这些真值假值的gotcha（JJ--??）。
...
针对`object`/`函数`/`array`和标量基本类型值（`string`，`number`和`boolean`）的相等比较，ES5规范的11.9.3.8-9一节有如下规定：
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">你可能注意到了，这些条款只提及字符串和数字，没有涉及<b>boolean</b>值。因为我们之前引用了条款11.9.3.6-7，其中规定了<b>boolean</b>值将首先被强制类型转换为数字。</td>
    </tr>
</table>
...
在`第三章`中我们介绍了“解封装（unboxing）”，即解封装包装基本数据类型的封装`object`（例如：`new String("abc"）`），然后返回其中的基本数据类型值（`"abc"`）。`==`中的`ToPromitive`强制类型转换也涉及该操作：
...
`NaN`能够被封装为`number`类型的封装对象，但是当执行`==`中的解封装时，`NaN == NaN`返回`false`，因为`NaN`不等于`NaN`（见`第二章`）。
...
因此如果我们让`a.valueOf()`在每次调用时产生边缘效应，如第一次调用时返回`2`，第二次调用时返回`3`？要这样做很容易：
...
但是实际情况却并非如此。让我们细细探来。我们所知的`!`运算符做些什么事？它根据ToBoolean规则（同时反转奇偶校验位）将值显式强制类型转换为`boolean`值。所以在`[] == ![]`执行之前已经是这个样子了：`[] == false`。从前面的例子中我们知道`false == []`，所以最后的结果一点也不意外。
...
如果你感觉对这种情况还是没办法适应，那么你的困惑可能并非如你所想是来自于强制类型转换。实际上是数组的`ToPrimitive`的执行方式，其将数组转换为`string`。很可能你希望`[2].toString()`返回的不是`"2"`，`[null].toString()`返回的不是`""`。

但是如果不这样的话，这些`string`又应该被如何转换呢？我实在是想不出其他可行的办法。可能`[2]`应该被转换为`"[2]"`，但是这在其他情况下会显得很古怪。

你也许会觉得因为`String(null)`的结果是`"null"`，所以`String([null])`也应该返回`"null"`。这的确有道理。问题的根源就在于此。

隐式强制类型转换不是问题的根源。即使显式强制类型转换中也是将`[null]`转换为`""`。问题是，将`array`字符串化为对应的`string`是否合情理。所以问题的根源是`String([..])`相关的规则。或许根本就不应该将`array`字符串化？但是这样在其他情况下又会产生很多问题。
...
实际上空`string`和空格`string`可以转换为另一个值`NaN`。但是这样一定更好吗？`" " == NaN`这样的比较会总是失败，但是个中原因并未因此而体现得更清楚。
...
我想你不太有可能在你的程序中使用`== []`这样的`boolean`测试，除非你对此还是一头雾水。或许你应该使用`== ""`或者`== 0`，如：
...
ES5规范中的11.8.5节规定了“抽象关系比较”，分两个部分：一是比较双方都是`string`（后半部分），后者其他类型值（前半部分）。
...
该算法首先对两个值调用`ToPrimitive`转换，如果其中一个返回值不是`string`，则两个值均会根据`ToNumber`规则被强制类型转换为`number`，然后进行数字比较。
...
然而如果`<`比较中的两个值均是`string`，则根据字母顺序来比较：
...
`a`和`b`没有被强制类型转换为`number`，因为它们在调用`ToPrimitive`后返回`string`。因此我们比较的是`"42"`和`"043"`这两个字符串，分别以`"4"`和`"0"`开头。因为`"0"`在字母顺序上小于`"4"`，所以最后的结果为`false`。
...
为什么`a == b`结果不是`true`呢？它们的`string`值相同（`"[object Object]"`），冒似应该相等才对？实际上不是这样。回忆一下前面介绍过的`object`引用的`==`比较吧。
...
`隐式`强制类型转换则不那么明显，而是作为其他操作的边缘效应。冒似它是`显式`强制类型转换的相反面，所以让人觉得是不好的（确实有很多人这样认为！），实际上`隐式`强制类型转换也能够提高代码的可读性。
...
表达式`a + 3`本身没有边缘效应，诸如改变`a`的值之类。它返回结果值`5`，它是语句`b = a + 3`赋值给变量`b`的值。
...
很多程序员会错误地认为变量`b`和`a`的值一样都是`43`。问题出在没有完全理解`++`操作符的边缘效应的产生时机。
...
当`++`出现在操作数前面时，如：`++a`，它的边缘效应（对`a`做递增）发生在表达式返回其结果值之前，而`a++`的边缘效应则发生在之后。

<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">你觉得++a++是合法的语法吗？你试过就知道它会产生一个ReferenceError错误，为什么呢？因为操作符产生的边缘效应需要赋值给一个变量。以++a++为例，a++部分首先执行（根据操作符优先级，见下），它在递增之前返回a的值42。然后试图执行++42，这会产生ReferenceError错误，因为++无法直接在42这样的值上产生边缘效应。</td>
    </tr>
</table>

有时我们会错误地认为可以使用括号`( )`将`a++`的边缘效应封装起来，例如：
...
可惜和我们想的不同，`( )`本身并不是一个封装表达式，不会在表达式`a++`产生边缘效应之后执行。实际上即便和我们想的一样，首先`a++`返回`42`，除非有另外一个表达式在`++`执行之后再次对`a`进行运算，否则还是不会得到`43`这个结果，`b`也不会被赋值为`43`。
...
操作符边缘效应的另一个例子是`delete`。我们在`第二章`中介绍过，我们用`delete`来删除`object`的属性，或者`array`的单元。通常它是以独立语句的形式出现：
...
如果操作有效／允许，`delete`返回结果`true`，否则返回`false`。它的边缘效应则是属性被从`object`中删除（或者单元被从`array`中删除）。
...
操作符边缘效应的最后一个例子是`=`赋值操作符，它也许会让人觉得即显而易见又晦涩难懂。
...
`a = 42`中的`=`看起来似乎不象是一个产生边缘效应的操作符。但是如果我们检查一下语句`a = 42`的结果值就会发现，它返回的是被赋值的`42`，所以赋值给`a`的值也就是`=`产生的边缘效应。

<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/suggestion.png'></td>
        <td style="border: none;">边缘效应同样适用于+=和-=这样的组合赋值操作符。例如，a = b += 2首先执行b += 2（即b = b + 2），该赋值操作的结果被赋值给a。</td>
    </tr>
</table>
...
在这里，`c = 42`执行结果为`42`（边缘效应是`42`被赋值给`c`），然后`b = 42`执行结果为`42`（边缘效应是`42`被赋值给`b`），最后`a = 42`被执行（边缘效应是`42`被赋值给`a`）。
...
很多程序员喜欢这样做，完全没问题。但是我们可以利用赋值语句的边缘效应，将两个`if`语句合二为一：
...
我们怎么知道上面是一个`object`常量呢？因为大括号`{ .. }`中的值被赋值给`a`。
...
很多人认为这里的`{ .. }`只是一个孤立的`object`常量，没有被赋值。但实际上完全不是这样。
...
实际上不是这样！`{"a":42}`这样的JSON数据本身没有任何问题，但是在执行时会产生JS错误，因为它被当作带有一个非法标签的语句块来执行。但是`foo({"a":42})`则没有问题，因为`{"a":42}`在其中是一个传递给`foo(..)`的`object`常量。因此确切地说，*JSON-P是将JSON转换为合法的JS语法*。
...
第一行`+`操作符语句中出现了`{}`，因此被当作一个值（一个空`object`）来处理。`第四章`中我们介绍过`[]`会被强制类型转换为`""`，因此`{}`也被强制类型转换为`string`：`"[object Object]"`。

但在第二行中，`{}`被当作一个独立的空代码块（不做任何事情）。代码块结尾不需要分号，所以在这里没有问题。最终表达式`+ []`将`[]`显式强制类型转换为`number`，即`0`。

#####对象分拆

从ES6开始，`{ .. }`出现的另一种情况是“分拆赋值”（详情请参见本系列的*《ES6 & Beyond》*），特别是`object`的分拆。例如：
...
因为操作符`,`的优先级比`=`低。所以`b = a++, a`被当作`(b = a++), a`来执行。因为（我们之前介绍过）`a++`有后续的边缘效应，所以赋给`b`的值是`++`对`a`做递增前的值`42`。
...
`opts && opts.cool`中的`opts`判断就象一个守卫，如果`opts`未被赋值（或者不是一个`object`），表达式`opts.cool`会产生错误。所以`opts`判断确保在这种情况下`opts.cool`不会被执行，因此不会产生错误！
...
但是执行顺序是从左到右还是从右到左又有什么关系呢？因为表达式可能会有边缘效应，比如函数调用：
...
赋值对象必须是一个标志符（或者ES6中的产生多个标志符的分拆表达式），因此这里的`42`是非法的，会报错：
...
语句和表达式在英语语言中有对应的称谓－语句就象句子，而表达式就象短句。表达式可以是简单／自包含的，也可能有边缘效应。

JavaScript语法规则中，单纯的语法规则之上是语义规则（也称作上下文）。例如，`{ }`在不同的情况下使用意义不同，可以是语句块，`object`常量，分拆赋值（ES6），或者命名函数参数（ES6）。
