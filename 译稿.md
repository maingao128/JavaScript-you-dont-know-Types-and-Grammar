...
表达式中包含多个运算符的时候，我们需要了解它们的执行顺序规则，这样才能搞明白上面例子的返回值。
...
这里的赋值语句用`( )`是必要的，原因是`&&`的优先级高于`=`，如果没有`( )`来绑定（bind）其中的表达式，代码会这样来执行`(str && matches) = str.match..`。这样会出错，因为`(str && matches)`的结果并不是一个变量，而是一个`undefined`值，它不能够出现在`=`的左边！
...
应该没有人会这样来写代码，本例是用来说明多个运算符串联时可能出现的一些常见问题。

它的结果是`42`。但这还不够，我们需要搞明白这个结果是怎样得来的。
