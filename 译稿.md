...
换句话说`真值`列表可以无限大，因而无法对其进行明确的定义，所以我们只能定义一个`假值`列表作为参照。
...
方法调用`a.toString()`非常明显（“toString”意指“to a string”），不过其中涉及一些隐蔽的隐式转换。因为`toString()`不能在`42`这样的基本类型值上使用，所以JS引擎会自动为`42`创建一个封装对象（见`第三章`），然后在该对象上调用`toString()`。这里的显式转换中含有隐式转换。
