...
很诡异是吧？确实。这也是强制类型转换和`==`被诟病的原因之一。但这并不是JavaScript本身的问题，而是我们这些*程序员*。不要有这样的想法，既“编程语言应该设法防止程序员犯错”。
...
如果让`a.valueOf()`在每次调用时都产生边际效应，如第一次调用时返回`2`，第二次调用时返回`3`，就会出现这样的情况。很简单：
