...
参数被转换为`boolean`后，再使用`Number(..)`进行*显式*强制类型转换以确保返回值为`0`或`1`。

是不是在这里*显式*强制类型转换要更好一些？如代码注释所说，它的确能够避免`NaN`带来的问题。但这最终取决于我们自身的需要。我觉得前者，即*隐式*强制类型转换，更为简洁（前提是我们不会传递`undefined`和`NaN`这样的值），而*显式*强制类型转换会带来一些冗余。

总之如本书一贯强调的那样，这一切都取决于我们自己的判断和权衡。
