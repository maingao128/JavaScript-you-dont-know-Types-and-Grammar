...
反方则认为这里面有太多坑，特别是对于缺乏经验的初学者来说，自动插入`;`会无意中改变代码的逻辑。还有一些程序员认为忽略分号本身就是错误的，他们倾向使用linter等工具来发现这些错误，而不是依赖于JS引擎。
...
换个角度来看，ASI实际上是将换行符作为有意义的“空格”来处理。其他语言中，如Python，空格是有意义的。但是将JavaScript中的新行视为有意义是否合适呢？
