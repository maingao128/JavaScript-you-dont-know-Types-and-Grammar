...
从ES6开始加入了一个新的基本数据类型－“标志”（Symbol）。标志是一种特殊的具有唯一性的值（不绝对保证唯一！），可以用做对象属性名且不容易产生命名冲突。该类型的引入主要是考虑到ES6中的一些特殊构造，不过你也可以自己定义标志。
...
你可以使用`Symbol(..)`原生构造函数来定义自己的标志。它很独特，对其不能使用`new`，否则会产生错误：
...
虽然标志实际上是非私有的（用`Object.getOwnPropertySymbols(..)`可以公开获得对象中的所有标志），但是我们主要还是将其用于私有和特殊属性的情况。大多数程序员将其用来替换属性名称中的下划线前缀，下划线前缀命名规范用来声明私有和特殊属性。

<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">标志并非对象，它们是简单标量基本类型值。</td>
    </tr>
</table>
