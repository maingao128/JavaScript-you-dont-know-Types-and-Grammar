...
简单值（即标量基本值，scalar primitives）*总是*通过值拷贝的方式赋值／传递，包括：`null`，`undefined`，`string`，`number`，`boolean`，和ES6中的`symbol`。

复合值，既`object（对象）`（包括`数组`，所有的封装对象，见`第三章`）和`function（方法）`，*总是*通过引用拷贝的方式来赋值／传递。

在前面的示例代码中，由于`2`是一个标量基本值，则变量`a`持有的是该值的一个拷贝，`b`被赋值为该值的另一个拷贝。`b`中的值更改时`a`中的值保持不变。
