...
我们使用`typeof`运算符来检测值的类型，它返回上述类型的字符串值。奇怪的是，以上七种类型和它们的字符串值并不一一对应：
...
`~`运算符（即字位操作“非”）也涉及强制类型转换，它令人费解，又常常被忽视。以至于理解它的程序员也对其避而远之。秉承本书的一贯宗旨，让我们来深入探讨一下`~`有哪些用途。
...
就象一元运算符`+`将值强制类型转换为数字（见前面的介绍），一元运算符`!`显式地将值强制类型转换为`boolean`。*问题*是它同时还将正值反转为负值（或者反之）。所以JS程序员经常使用`!!`运算符将值显式强制类型转换为`boolean`值，因为第二个`!`会将结果反转为原值：
...
这很能说明问题，即不管*隐式强制类型转换*如何被诟病，也仍然有它适用的地方。
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">之所以说<b>a || b</b>大致等价于（roughly equivalent）<b>a ? a : b</b>是因为它们的返回结果相同，但是它们之间存在一个细微的差别。在<b>a ? a : b</b>中，如果<b>a</b>是更为复杂一些的表达式（比如：一个有边际效应的函数调用，等等），则<b>a</b>表达式有可能被执行两次（如果第一次结果为真）。相反，对于<b>a || b</b>，表达式只会执行一次，其结果值会被用于条件判断以及返回值（如果适用的话）。<b>a &amp;&amp; b</b>和<b>a ? b : a</b>也一样。</td>
    </tr>
</table>
...
通过这样的方式来设置`缺省值`很普遍，也很实用。甚至那些公开诟病JavaScript强制类型转换的程序员也在使用。

`&&`的情况又如何？

有一种方式对于程序员不太常见，但经常被JS代码压缩工具使用。既如果第一个操作数为真值，`&&`运算符“选择”第二个操作数的值作为返回值，这种方式也称为“守护运算符”（见`第五章`中`第140页上的“短路”`），第一个表达式“守护”第二个表达式：
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;"><b>a = b || "something"</b>和<b>a &amp;&amp; b()</b>中使用到了“短路”机制，我们将在第五章第140页上的“短路”一节中详细介绍。</td>
    </tr>
</table>
...
宽松相等运算符是`==`，严格相等运算符是`===`。两者均用于判断两个值是否“相等”，但是宽松相等和严格相等在判断”相等“时有一个很重要的区别。
...
如果你需要强制类型转换，则使用`==`宽松相等运算符，如果不需要则使用`===`严格相等运算符。
...
运算符`==`的工作机制在ES5规范中第11.9.3节“抽象相等比较算法”中定义。其中包括一个全面而简单的算法，针对所有可能出现的类型组合，以及每个组合如何执行（如果需要）强制类型转换。
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">"grammar"一词相对"syntax"对读者来说更陌生一些。很多情况下它们是相似的，都是对语言规则的定义。但也有一些细微的差别，不过对我们而言微不足道。JavaScript语法系统地规定了编码要遵循的各种语法规则（运算符、关键词等等）。换句话说，介绍语法而不涉及语法规则会导致很多细节被遗漏。所以本章重点介绍语法规则，虽然程序员直接打交道的是语言的语法。</td>
    </tr>
</table>
...
对于JavaScript的语法也是这样。语句相当于句子，表达式相当于短句，运算符则相当于标点符号和连词。
...
很多程序员会错误地认为变量`b`和`a`的值一样都是`43`。问题出在没有完全理解`++`运算符的边缘效应的产生时机。

递增运算符`++`和递减运算符`--`都是一元运算符（见`第四章`），它们可以出现在操作数前面或者后面：
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">你觉得++a++是合法的语法吗？你试过就知道它会产生一个ReferenceError错误，为什么呢？因为运算符产生的边缘效应需要赋值给一个变量。以++a++为例，a++部分首先执行（根据运算符优先级，见下），它在递增之前返回a的值42。然后试图执行++42，这会产生ReferenceError错误，因为++无法直接在42这样的值上产生边缘效应。</td>
    </tr>
</table>
...
然而我们还是有一个办法，即`,`系列语句逗号运算符。它让我们将多个独立的表达式语句变成一个语句：
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">上例中，我们需要将a++, a放在( .. )中。这样做是因为运算符优先级所致，我们将在本章后面介绍。</td>
    </tr>
</table>

表达式`a++, a`中的第二个语句表达式`a`在`a++`之后执行，结果为`43`，并被赋值给`b`。

运算符边缘效应的另一个例子是`delete`。我们在`第二章`中介绍过，我们用`delete`来删除`object`的属性，或者`array`的单元。通常它是以独立语句的形式出现：
...
运算符边缘效应的最后一个例子是`=`赋值运算符，它也许会让人觉得即显而易见又晦涩难懂。
...
`a = 42`中的`=`看起来似乎不象是一个产生边缘效应的运算符。但是如果我们检查一下语句`a = 42`的结果值就会发现，它返回的是被赋值的`42`，所以赋值给`a`的值也就是`=`产生的边缘效应。

<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/suggestion.png'></td>
        <td style="border: none;">边缘效应同样适用于+=和-=这样的组合赋值运算符。例如，a = b += 2首先执行b += 2（即b = b + 2），该赋值操作的结果被赋值给a。</td>
    </tr>
</table>
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">封装matches = str.match..的( .. )是必需的。原因我们在第137页中的“运算符优先级”会介绍。</td>
    </tr>
</table>
...
看起来似乎是`+`运算符根据第一个操作数的不同情况（`[]`或`{}`）产生不同的结果。但实际上与此无关！

第一行`+`运算符语句中出现了`{}`，因此被当作一个值（一个空`object`）来处理。`第四章`中我们介绍过`[]`会被强制类型转换为`""`，因此`{}`也被强制类型转换为`string`：`"[object Object]"`。
...
我们在`第四章`中介绍过，JavaScript中的`&&`和`||`有选择地返回它的其中一个操作数的值，而非返回`true`或`false`。我们很容易将其归结为一个运算符和两个操作数的情况：
...
要弄明白上面的例子返回什么结果，我们需要了解当表达式中有超过一个运算符时，它们执行的先后顺序规则。
...
因为运算符`,`的优先级比`=`低。所以`b = a++, a`被当作`(b = a++), a`来执行。因为（我们之前介绍过）`a++`有后续的边缘效应，所以赋给`b`的值是`++`对`a`做递增前的值`42`。

了解运算符优先级的必要性显而易见。如果你在一系列语句中使用`,`，请记住它的优先级是最低的。其他的操作数的优先级都比它高(JJ--?? Every other operator will more tightly bind than , will.)。
...
每个语言都有自己的运算符优先级。但令人吃惊的是很少有JS程序员了解过这方面的细节。
...
<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;">遗憾的是，JS规范中对运算符优先级并没有一个统一的介绍。你需要从语法规则中逐一了解。所以在这里我们尽量列出那些常见和有用的。完整的优先级列表在<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Operator_Precedence">MDN网站</a>的“优先级列表”中。</td>
    </tr>
</table>

####短路

在`第四章`中，我们在旁注中提到`&&`和`||`等运算符的“短路”特性。现在让我们来深入了解一下。

对于运算符`&&`和`||`，如果从左边的操作数足以得出结果，则右边的操作数可以被略过。因此我们称之为“短路”（即执行最短路径）。
...
让我们回头看一看之前的一个复杂的例子，很多运算符串联在一起，特别是`? :`三元运算符。运算符`? :`的优先级是否比`&&`和`||`高？
...
运算符`&&`和`||`首先绑定，然后是`? :`。但如果出现相同优先级的多个运算符时怎么办？它们是否按从左到右或者从右到左的顺序执行？

通常运算符不是从左到右就是从右到左关联，取决于组合（grouping）是从左到右还是从右到左。
...
但是其并非在任何时候都是确切的。有一些运算符在左关联的情况下的表现和右关联完全不同。
...
右关联（grouping）的另一个例子是`=`运算符。本章前面我们举过串联赋值的例子：
...
我个人的观点，是这里有一个重要的中间地带。我们需要同时依赖运算符优先级／关联性以及人为使用`( )`来控制组合，在`第四章`中我对隐式强制类型转换的使用也持这样的观点，安全合理地使用，而非无节制地滥用。

例如，我觉得自己对`if (a && b && c) ..`完全OK，就不会象`if ((a && b) && c) ..`这样显式地表示出关联性，因为这样比较繁琐冗长。

从另一方面来说，如果需要将两个`? :`条件判断运算符串联起来，我会加上`( )`来人为控制组合，以便让代码逻辑更清晰易读。

因此我的建议和`第四章`类似：如果运算符优先级／关联性能够让代码更简洁，就使用之。如果人为加入`( )`有助于提高代码可读性从而避免困扰，就使用之。
...
除了`==`，`switch`中`true`和`true`之间的比较仍然是严格的。如果`case`表达式的结果为真值，但不是严格意义上的`true`（见`第四章`），则匹配不成立。如果在这里使用`||`和`&&`这样的“逻辑运算符”，你会踩到陷井：
...
JavaScript详细定义了运算符的优先级（绑定运算符的先后顺序）和关联性（表达式中的多个运算符如何组合）。一旦你掌握了这些规则，你就能够自己判断是否使用它们来让代码更佳简洁明了，或者因为它们过于隐秘而应避免使用。
