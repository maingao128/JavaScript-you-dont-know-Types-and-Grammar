...
答案是`a ? b : (c ? d : e)`。和`&&`和`||`的情况不同，右关联在这里会影响到返回结果，因为`(a ? b : c) ? d : e`对某些值的组合（并非所有）的处理方式会有不同。
...
这里我们得到的结果相同，implies that the grouping is moot(JJ--??)。然而：
