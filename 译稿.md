...
以上24种情况中有17种比较好理解。比如我们知道`""`和`NaN`不相等，`"0"`和'0'相等。

然而有7种情况加了注释“晕！”，因为它们属于假阳（false positive）的情况，里面坑很多。`""`和`0`明显是两个不同的值，它们之间的强制类型转换很容易搞错。请注意这里不存在假阴（false negatives）的情况。
...
前面列举的7个坑基本上涵盖了我们可能碰到的*正常*值间强制类型转换的情况（除了修改`valueOf()`和`toStrign()`的情况以外）。
