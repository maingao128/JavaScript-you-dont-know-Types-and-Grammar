...
对于`字符串`值，`ToNumber`基本遵照`数字`常量相关规则／语法（见`第三章`）。如果失败则返回`NaN`（在处理`数字`常量失败时会产生语法错误）。`ToNumber`的另一个不同之处是以`0`开头的十六进制数并不按十六进制处理（而是按十进制，见`第二章`）。

<table>
    <tr style="border: none;">
        <td style="border: none; width: 150px;"><img src='images/note.png'></td>
        <td style="border: none;"><b>数字常量</b>的语法规则与<b>ToNumber</b>对<b>字符串</b>的操作规则之间差别很小，这里不做进一步介绍，可参考ES5规范第9.3.1节。</td>
    </tr>
</table>

对象（以及数组）会首先被转换为相应的基本类型值，然后结果（为基本类型，但还不是`数字`）再根据上述规则被强制转换为`数字`。

为了进行基本类型值转换，抽象操作`ToPrimitive`（见ES5规范第9.1节）会先（通过内部操作`DefaultValue`，见ES5规范第8.12.8节）检查待转换的值，看其是否包含`valueOf()`方法。如果有`valueOf()`并且返回基本类型值，则使用该值进行强制类型转换。如果没有`valueOf()`方法，则使用`toString()`的返回值（如果有的话）来进行强制类型转换。如果
